...
Public Class FormBatchPrint
    ...
    Private Sub Btn开始_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn开始.Click
        ...
        If lvw文件列表.Items.Count = 0 Then
            MessageBox.Show(”未添加工程图文件。“, AIPTool, MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
            ...
            If chk存为dwg.Checked = True Then
                ...
                If str另存到子文件夹 = "1" Then
                    ...
                    If IsDirectoryExists(strChildDirectory) = False Then
                        IO.Directory.CreateDirectory(strChildDirectory)
                    End If
                    strDwgFullFileName = IO.Path.Combine(strChildDirectory, GetFileNameInfo(strInventorDrawingDocumentFullFileName).AddStringName & DWG)
                Else
    ...
    Private Sub Btn添加文件_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn添加文件.Click
        ...
        Dim strFilter As String = "Autodesk Inventor 工程图(*.idw,*.DWG)|*.idw,*.DWG" '添加过滤文件
    ...
    Private Sub Btn添加文件夹_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn添加文件夹.Click
        ...
        GetAllFile(strDestinationFolder, lvw文件列表, DWG)
    ...
    Private Sub LoadBOMSub(ByVal oBOMRows As BOMRowsEnumerator, ByVal olistiview As ListView)
        ...
        For Each oBOMRow As BOMRow In oBOMRows
            ...
            Dim strInventordwgDrawingFullFileName As String
            strInventordwgDrawingFullFileName = GetChangeExtension(strDocumentFullFileName, DWG)

            If IsFileExists(strInventorDrawingFullFileName) = False Then
                If IsFileExists(strInventordwgDrawingFullFileName) = False Then
                    '如果工程图和dwg都不存在，则跳过
                    Continue For
                ElseIf IsItemInListView(olistiview, strInventordwgDrawingFullFileName) = False Then
                    olistiview.Items.Add(strInventordwgDrawingFullFileName)
                End If

            ElseIf IsItemInListView(olistiview, strInventorDrawingFullFileName) = False Then
                ...
                Select Case File_Attribute.Attributes
                    ...
                    Case Else
                        If IsItemInListView(lvw文件列表, strInventorDrawingFullFileName) = False Then
                            ...
                            ElseIf GetFileExtensionLCase(strInventorDrawingFullFileName) = DWG Then
                                lvw文件列表.Items.Add(strInventorDrawingFullFileName)
