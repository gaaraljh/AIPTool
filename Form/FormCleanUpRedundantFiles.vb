...
Public Class FormCleanUpRedundantFiles
    ...
    Private Sub 查询ToolStripButton_Click(sender As Object, e As EventArgs) Handles 查询ToolStripButton.Click
        ...
        Try
            ...
            If Not Directory.Exists(strInventorAssemblyDocumentNamePath) Then
                MessageBox.Show($"目录不存在: {strInventorAssemblyDocumentNamePath}", AIPTool, MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return
            End If
        ...
        'strInventorDWGDrawingDocumentFullFileName = GetChangeExtension(strInventorAssemblyDocumentFullFileName, DWG)
        If IsFileExists(strInventorDrawingDocumentFullFileName) = True Then
            ...
            '    strUseDocumentList.Add(Strings.UCase(strInventorDWGDrawingDocumentFullFileName))
        End If
        ...
        For Each oListViewItem As ListViewItem In lvw文件列表.Items
            ...
            If strUseDocumentList.Contains(strFullFileName) = True Then
                ...
                If GetFileExtensionUCase(strFullFileName) = IDW Then
                ...
                ElseIf GetFileExtensionUCase(strFullFileName) = DWG Then
                    strInventorDocumentFullFileName = GetChangeExtension(strFullFileName, IAM)
    ...
    Private Sub 清理ToolStripButton_Click(sender As Object, e As EventArgs) Handles 清理ToolStripButton.Click
        ...
        For Each oListViewItem As ListViewItem In lvw文件列表.Items
            ...
            strinventorDrawingDocumentFullFileName = GetChangeExtension(strFullFileName, IDW)
            strinventorDWGDrawingDocumentFullFileName = GetChangeExtension(strFullFileName, DWG)

            If oListViewItem.Checked = True And IsFileExists(strFullFileName） = True Then

                If IsFileExists(strinventorDrawingDocumentFullFileName) = True Then
                    AddOldExtension(strFullFileName)
                    AddOldExtension(strinventorDrawingDocumentFullFileName)
                    oListViewItem.Remove()
                End If

                If IsFileExists(strinventorDWGDrawingDocumentFullFileName) = True Then
                    AddOldExtension(strFullFileName)
                    AddOldExtension(strinventorDWGDrawingDocumentFullFileName)
                    oListViewItem.Remove()
                End If
            Else
            ...
            End If
        Next
        MessageBox.Show("清理冗余文件完成。", AIPTool, MessageBoxButtons.OK, MessageBoxIcon.Information)
