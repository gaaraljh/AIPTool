...
Public Class FormFormatConversion
    ...
    Private Sub AddFilesInListView(ByVal oListView As ListView, ByVal oFileList As List(Of String),
                                      ByVal IsContainIdw As Boolean, ByVal IsContaindwg As Boolean, ByVal IsContainIpt As Boolean)
    ...
    Private Sub 添加文件ToolStripButton_Click(sender As Object, e As EventArgs) Handles 添加文件ToolStripButton.Click
        ...
        Dim strFilter As String = "Autodesk Inventor文件(*.idw;*.dwg;*.iam;*.ipt)|*.idw;*.dwg;*.iam;*.ipt"


        If 零部件ToolStripButton.Checked = True And 工程图ToolStripButton.Checked = True Then
            strFilter = "Autodesk Inventor文件(*.idw;*.dwg;*.iam;*.ipt)|*.idw;*.dwg;*.iam;*.ipt" '添加过滤文件
        ...
        ElseIf 工程图ToolStripButton.Checked = True Then
            strFilter = "Autodesk Inventor 工程图(*.idw;*.dwg)|*.idw;*.dwg" '添加过滤文件
        End If
        ...
        AddFilesInListView(Lvw文件列表, oFileList, 工程图ToolStripButton.Checked, 工程图ToolStripButton.Checked, 零部件ToolStripButton.Checked)
    ...
    Private Sub 添加文件夹ToolStripButton_Click(sender As Object, e As EventArgs) Handles 添加文件夹ToolStripButton.Click
        ...
        If 工程图ToolStripButton.Checked = True Then
            strdwgExtension = DWG
            oFileList = GetAllFilesByExtension(strDestinationFolder, strdwgExtension)
    ...
    Private Sub 浏览ToolStripButton_Click(sender As Object, e As EventArgs) Handles 浏览ToolStripButton.Click
        ...
        If folderBrowserDialog.ShowDialog() = DialogResult.OK Then
            ...
            指定文件夹ToolStripTextBox.Text = selectedFolderPath
    ...
    Private Sub FrmSaveAs_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.Icon = My.Resources.AIPTool48
        ...
        黑色PDFToolStripButton.Image = My.Resources.清除随机颜色16.ToBitmap
    ...
    Private Sub Lvw文件列表_DragDrop(sender As Object, e As DragEventArgs) Handles Lvw文件列表.DragDrop
        ...
        For Each strDestinationFileFolder As String In filePaths
            ...
            If IO.Directory.Exists(strDestinationFileFolder) Then
                ...
                If 工程图ToolStripButton.Checked = True Then
                    strDWGExtension = DWG
                    oFileList = GetAllFilesByExtension(strDestinationFileFolder, strDWGExtension)
    ...
    Private Sub 导入已打开的文件ToolStripButton_Click(sender As Object, e As EventArgs) Handles 导入已打开的文件ToolStripButton.Click
        ...
        For Each oInventorDocument In ThisApplication.Documents.VisibleDocuments
            ...
            If IsItemInListView(Lvw文件列表, strInventorDocumentFullFileName) = False Then
                ...
                If GetFileExtensionLCase(strInventorDocumentFullFileName) = DWG And 零部件ToolStripButton.Checked = True Then
                    Lvw文件列表.Items.Add(strInventorDocumentFullFileName)
                End If
            End If
    ...
    Private Sub 导入当前部件ToolStripButton_Click(sender As Object, e As EventArgs) Handles 导入当前部件ToolStripButton.Click
        ...
        LoadBOM(oInventorAssemblyDocument, Lvw文件列表, 工程图ToolStripButton.Checked, 工程图ToolStripButton.Checked, 零部件ToolStripButton.Checked)
    ...
    Private Sub LoadBOM(ByVal oInventorAssemblyDocument As AssemblyDocument, ByVal olistiview As ListView,
        ...
        For Each oBOMView As BOMView In oBOM.BOMViews
            If oBOMView.ViewType = BOMViewTypeEnum.kStructuredBOMViewType Then
                '遍历这个bom页面
                LoadBOMSub(oBOMView.BOMRows, olistiview, IsContainIdw, IsContaindwg, IsContainIpt)
                Exit For
            End If
        Next
    ...
    Private Sub LoadBOMSub(ByVal oBOMRows As BOMRowsEnumerator, ByVal olistiview As ListView,
        ...
        For Each oBomRow As BOMRow In oBOMRows
            ...
            If IsContaindwg = True Then
                Dim strdwgDrawingFullName As String
                strdwgDrawingFullName = GetChangeExtensionDocument(strInventorDocumentFullFileName, DWG)
                If strdwgDrawingFullName <> "" Then
                    strInventorDocumentFullFileNames.Add(strdwgDrawingFullName)
                End If
            End If

        Next

        AddFilesInListView(olistiview, strInventorDocumentFullFileNames, IsContainIdw, IsContaindwg, IsContainIpt)

        For Each oBomrow As BOMRow In oBOMRows
            '遍历下一级
            If (oBomrow.ChildRows IsNot Nothing) Then
                LoadBOMSub(oBomrow.ChildRows, olistiview, IsContainIdw, IsContaindwg, IsContainIpt)
            End If
...
999:
    ...
    Private Sub 开始转换ToolStripButton2_Click(sender As Object, e As EventArgs) Handles 开始转换ToolStripButton2.Click
        ...
        If Lvw文件列表.Items.Count = 0 Then
            MessageBox.Show(”未添加文件。“, AIPTool, MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
        ...
        For Each oListViewItem As ListViewItem In Lvw文件列表.Items
            ...
            Dim intAllBlack As Integer = 0

            If 黑色PDFToolStripButton.Checked = True Then
                intAllBlack = 1
            End If
                ...
                Select Case oInventorDocument.DocumentType
                    Case kDrawingDocumentObject
                        If DWGToolStripButton.Checked = True Then
                            ...
                            strFormatFulFileName = IO.Path.Combine(oFormatFileNameInfo.Folder, oFormatFileNameInfo.AddStringName & DWG)

                            DrawingSaveAsDwgSub(oInventorDocument.FullDocumentName, strFormatFulFileName, 替换ToolStripButton.Checked)
                        ...
                        If DXFToolStripButton.Checked = True Then
                            ...
                            DrawingSaveAsPdfSub(oInventorDocument.FullDocumentName, strFormatFulFileName, 替换ToolStripButton.Checked， intAllBlack)
                        End If
...
999:
        ...
        MessageBox.Show(”格式转换完成。“, AIPTool, MessageBoxButtons.OK, MessageBoxIcon.Information)
